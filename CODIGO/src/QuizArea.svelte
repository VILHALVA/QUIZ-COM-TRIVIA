  ...
  function handleAnswerChoice(e = {}) {
    if (!representation[questionNo].answered) {
      ...
      if (questionNo === 9) {
        ...
        if (score < 5) {
          finalMessage = '😵Você está nas drogas?';
        } 
        else if (score === 5) {
          finalMessage = "🤓Não desista, tente mais.";
        } 
        else {
          finalMessage = "🔥Você está no fogo!!!";
        }
      }
...
<div id="main" in:fadeIn out:fadeOut>
  ...
  {#if representation.length > 0 && !resultsScreen}
    ...
    {#if buttonBarVisibility}
      <div id="button-bar">
        {#if questionNo < 9}
          <button value="Next" on:click={() => handleClick('f')}>PROXIMO</button>
        {/if}
        {#if questionNo > 0}
  ...
  {:else if resultsScreen}
    <div id="results">
      ...
      <p style="font-size: 24px">Atualize a página para jogar novamente!</p>
    </div>
  {:else}
    <span
